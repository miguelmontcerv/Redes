...
void manejador_paquetes(u_char *param, const struct pcap_pkthdr *header, const u_char *pkt_data);
int main()
{
	printf("\n.: Lista de interfaces :.\n\n");
	pcap_if_t *alldevs;
	...
	pcap_loop(adhandle, 1, manejador_paquetes, (unsigned char *)dumpfile);
...
void manejador_paquetes(u_char *dumpfile, const struct pcap_pkthdr *header, const u_char *pkt_data)
{
	...
	/* guardamos el paquete en el dump file */
